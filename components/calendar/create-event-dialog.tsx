...
export function CreateEventDialog({
  ...
  const { addEvent, updateEvent, goToDate } = useCalendarStore();

  // Initial state based on eventToEdit if present, otherwise defaults
  ...
  // Effect to populate form when opening with eventToEdit
  useEffect(() => {
    if (open && eventToEdit) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onOpenChange(false);
    // State reset is handled by effect on next open
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {eventToEdit ? "Update event details." : "Add a new event to your calendar. Fill in the details below."}
          </DialogDescription>
        </DialogHeader>
