...
export const useCalendarStore = create<CalendarState>((set, get) => ({
  ...
  addEvent: (event: Omit<Event, "id">) => {
    ...
    set((state) => ({ eventsVersion: state.eventsVersion + 1 }));
  },

  deleteEvent: (id: string) => {
    const { deleteEvent } = require("@/mock-data/events");
    deleteEvent(id);
    set((state) => ({ eventsVersion: state.eventsVersion + 1 }));
  },

  updateEvent: (event: Event) => {
    const { updateEvent } = require("@/mock-data/events");
    updateEvent(event);
    set((state) => ({ eventsVersion: state.eventsVersion + 1 }));
  },
  ...
  getCurrentWeekEvents: () => {
    const state = get();
    // Depend on eventsVersion to trigger re-calculation if memoized (not here)
    // or to ensure state change triggers component re-render.
    const _v = state.eventsVersion;
